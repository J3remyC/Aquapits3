...
var options = {
  strings: ["Setup", "Install", "Clean", "Maintain", "Aquascape", "Design"],
  typeSpeed: 40,
  backSpeed: 20,
  backDelay: 2000,
  loop: true,
...
function calculateCost() {
  const cleaningSetup = document
    .getElementById("cleaningSetup")
    .value.split(",")
    .map(Number);
  const repairType = document
    .getElementById("repairType")
    .value.split(",")
    .map(Number);
  const reglassType = document
    .getElementById("reglassType")
    .value.split(",")
    .map(Number);
  ...
  const totalCostMin =
    cleaningSetup[0] + repairType[0] + reglassType[0] + distance[0];
  const totalCostMax =
    cleaningSetup[1] + repairType[1] + reglassType[1] + distance[1];

  const validCostMin = isNaN(totalCostMin) ? 0 : totalCostMin;
  const validCostMax = isNaN(totalCostMax) ? 0 : totalCostMax;

  document.getElementById("totalCost").textContent =
    "₱" + validCostMin.toLocaleString("en-PH");
  document.getElementById("totalCostMax").textContent =
    "₱" + validCostMax.toLocaleString("en-PH");
}
...
function sendMail() {
    ...
    emailjs.send("service_emv5iqe", "template_98vajbz", parms).then(
      function (response) {
        alert("Email Sent!");
  
        // Clear the input fields after successful email submission
